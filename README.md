...
The bot integrates with the LicenseChain Core API ([LicenseChain API](https://github.com/LicenseChain/api)). **Base URL:** `https://api.licensechain.app/v1`.
...
docker run --name lc-tg-bot \
  ...
  -e LICENSECHAIN_API_KEY=your_licensechain_api_key \
  -e LICENSECHAIN_BASE_URL=https://api.licensechain.app/v1 \
...
- License commands failing:
  - verify `LICENSECHAIN_API_KEY` (legacy `LICENSE_CHAIN_API_KEY` is also accepted)
