...
// On Vercel, no filesystem for logs; skip File transports to avoid ENOENT mkdir 'logs'
...
const useWebhook = USE_WEBHOOK || isVercel;
const licenseChainApiKey =
  process.env.LICENSECHAIN_API_KEY ||
  process.env.LICENSE_CHAIN_API_KEY;
const licenseChainBaseUrl =
  process.env.LICENSECHAIN_BASE_URL ||
  process.env.LICENSE_CHAIN_API_URL ||
  'https://api.licensechain.app/v1';

// Create Telegram bot (polling only when not using webhook)
const bot = new TelegramBot(process.env.TELEGRAM_TOKEN, { polling: !useWebhook });

// Initialize bot components
const licenseClient = new LicenseChainClient({
  apiKey: licenseChainApiKey,
  baseUrl: licenseChainBaseUrl
});
...
async function initializeBot() {
  ...
  initializeBotPromise = (async () => {
    ...
    if (!licenseChainApiKey) {
      logger.warn('LICENSECHAIN_API_KEY is not set; legacy LICENSE_CHAIN_API_KEY is also accepted, but LicenseChain API features may not work until one of them is configured.');
    }
...
// Webhook endpoint (only used when USE_WEBHOOK=true or Vercel)
if (USE_WEBHOOK || process.env.VERCEL) {
...
// On Vercel, the root is sometimes invoked as a serverless function; export a handler so it doesn't error.
// api/webhook.js and api/health.js use the attached bot and initializeBot.
if (process.env.VERCEL) {
  const vercelHandler = (req, res) => { res.status(404).end(); };
  vercelHandler.bot = bot;
  vercelHandler.initializeBot = initializeBot;
