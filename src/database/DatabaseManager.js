...
function getConnectionConfig() {
  ...
  // Serverless: avoid holding stale connections; pooler may close idle conns → ECONNRESET
  if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
    opts.max = 1;
    opts.idleTimeoutMillis = 1000;
...
class DatabaseManager {
  ...
  _isConnectionError(err) {
    ...
    return code === 'ECONNRESET' || code === 'ECONNREFUSED' || code === 'ETIMEDOUT' ||
      code === '08006' || code === '57P01' || msg.includes('connection') || msg.includes('socket hang up');
  }
  ...
  async getValidationCount(platformUserId = null, startDate = null) {
    ...
    const r = await this._query(query, params);
  ...
  async getBotStats() {
    const [users, licenses, commands] = await Promise.all([
      this._query(`SELECT COUNT(*)::int AS c FROM ${PREFIX}users`),
      this._query(`SELECT COUNT(*)::int AS c FROM ${PREFIX}licenses`),
      this._query(`SELECT COUNT(*)::int AS c FROM ${PREFIX}commands`)
    ]);
    return {
  ...
  async setBotStatus(status, updatedBy) {
    await this._query(`DELETE FROM ${PREFIX}bot_status`);
    await this._query(
